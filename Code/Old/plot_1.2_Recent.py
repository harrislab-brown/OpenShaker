...
def process_sweep_data(filename):
    ...
    for freq, group in grouped:
        ...
        # Planar-to-Z Ratio (non-dimensional)
        avg_bath_z = (rms_0_z + rms_2_z) / 2.0
        denom_z = max(avg_bath_z, 0.001)
        planar_to_z_ratio = avg_bath_planar / denom_z
        
        # Base Shaker Planar-to-Z Ratio (non-dimensional)
        # Note: For base shaker, Z motion is actually from Y direction (sensor orientation)
        denom_shaker_z = max(rms_4_y, 0.001)
        shaker_planar_to_z_ratio = shaker_planar / denom_shaker_z
        ...
        results.append({
            ...
            'Shaker_Planar_to_Z_Ratio': shaker_planar_to_z_ratio
        })
...
def generate_plots(df, csv_filename):
    ...
    # --- PLOT 5: Planar-to-Z Ratio [TOP MIDDLE] ---
    axs[0, 1].plot(freqs, df['Planar_to_Z_Ratio'], marker='o', color='#16a085', linewidth=2.5, label='Bath Planar / Bath Z')
    axs[0, 1].plot(freqs, df['Shaker_Planar_to_Z_Ratio'], marker='D', color='#2980b9', linewidth=2.5, label='Shaker Planar / Shaker Y')
    axs[0, 1].set_title("Planar-to-Vertical Ratio", fontsize=14, pad=10)
    axs[0, 1].set_xlabel("Frequency (Hz)", fontsize=12)
    axs[0, 1].set_ylabel("Planar / Vertical (non-dimensional)", fontsize=12)
    axs[0, 1].set_yscale('log')
    axs[0, 1].set_ylim(0.01, 100)
    axs[0, 1].grid(True, linestyle='--', alpha=0.7, which='both')
    axs[0, 1].legend(loc='best', frameon=True, shadow=True)
    ...
    avg_output_image = os.path.join(SECOND_OUTPUT_DIR, f"{os.path.splitext(base_name)[0]}_bath_z_vs_shaker_y.png")
